...
import { uploadTaskStepImageAction, deleteTaskStepImageAction } from "../step-image-actions";
import Image from "next/image";
import ConfirmModal from "@/components/ConfirmModal";

interface Props {
  stepId: string;
  initialThumbs: Array<string | null>;
  onThumbsChange?: (thumbs: Array<string | null>) => void;
}

export default function TaskStepImageSlots({ stepId, initialThumbs, onThumbsChange }: Props) {
  ...
  const handleFileSelect = async (position: number, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadTaskStepImageAction(formData);
      ...
      if (fileInputRefs.current[position - 1]) fileInputRefs.current[position - 1]!.value = "";
      if (cameraInputRefs.current[position - 1]) cameraInputRefs.current[position - 1]!.value = "";
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      formData.append("stepId", stepId);
      formData.append("position", position.toString());
      await deleteTaskStepImageAction(formData);
      const newThumbs = [...thumbs];
  ...
  const imagesCount = thumbs.filter(Boolean).length;
  const maxSlotsToShow = Math.min(imagesCount + 1, 3);

  return (
    <div className="space-y-2">
      <label className="block text-sm font-medium text-neutral-700">Fotos de referencia</label>
      {error && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-2 text-xs text-red-700">{error}</div>
      )}
      <div className="flex gap-3">
        {[1, 2, 3].slice(0, maxSlotsToShow).map((position) => {
          ...
          return (
            <div key={position} className="flex-shrink-0">
              <div className="relative w-28 h-28 rounded-lg border border-neutral-300 bg-neutral-50 overflow-hidden">
                {thumbUrl ? (
                  <>
                    <Image src={thumbUrl} alt={`Foto ${position}`} fill className="object-cover" sizes="112px" />
                    <button
                      type="button"
                  ...
                  <div
                    ...
                    onClick={() => !isUploading && setOpenSheetPosition(position)}
                    role="button"
                  >
                    {isUploading ? (
      ...
      <p className="text-xs text-neutral-500">Máximo 3 fotos de referencia por tarea</p>

      {[1, 2, 3].map((pos) => (
        <div key={`inputs-${pos}`} className="hidden">
          <input ref={(el) => { cameraInputRefs.current[pos - 1] = el; }} type="file" accept="image/*" capture="environment" onChange={(e) => handleFileSelect(pos, e)} />
          <input ref={(el) => { fileInputRefs.current[pos - 1] = el; }} type="file" accept="image/*" onChange={(e) => handleFileSelect(pos, e)} />
        </div>
      ))}
      ...
      {openSheetPosition !== null && (
        <>
          <div className="fixed inset-0 z-[70] bg-black/40" onClick={() => setOpenSheetPosition(null)} />
          <div className="fixed bottom-0 left-0 right-0 z-[80] bg-white rounded-t-2xl shadow-2xl pt-5 pb-10 px-4 space-y-1">
            ...
            <button type="button" onClick={() => { cameraInputRefs.current[openSheetPosition - 1]?.click(); setOpenSheetPosition(null); }} className="w-full flex items-center gap-3 px-4 py-4 rounded-xl text-base text-neutral-800 hover:bg-neutral-100 transition text-left">
              <svg className="w-5 h-5 text-neutral-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              Tomar foto
            </button>
            <button type="button" onClick={() => { fileInputRefs.current[openSheetPosition - 1]?.click(); setOpenSheetPosition(null); }} className="w-full flex items-center gap-3 px-4 py-4 rounded-xl text-base text-neutral-800 hover:bg-neutral-100 transition text-left">
              <svg className="w-5 h-5 text-neutral-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              Elegir de galería
            </button>
            <div className="pt-3">
              <button type="button" onClick={() => setOpenSheetPosition(null)} className="w-full py-3.5 text-base font-medium text-neutral-500 rounded-xl border border-neutral-200 hover:bg-neutral-50 transition">
                Cancelar
              </button>
            </div>
          </div>
