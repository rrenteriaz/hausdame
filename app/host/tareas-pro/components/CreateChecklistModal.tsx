...
export default function CreateChecklistModal({
  ...
  // open siempre inicia en false, por lo que document.body nunca se accede en SSR
  const modal = open ? createPortal(
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 sm:p-6">
      ...
      />
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden max-h-[calc(100dvh-2rem)] flex flex-col">
        <div className="px-6 py-4 border-b shrink-0">
          <h2 className="text-base font-semibold text-neutral-900">Nueva tarea</h2>
        </div>
        <form action={createTaskTemplate} className="px-6 py-5 space-y-4 overflow-y-auto">
          {/* Propiedad */}
          <div>
