...
export type ChecklistPropertyItem = {
  id: string;
  name: string;
  shortName: string | null;
};

export default function ChecklistPropertyList({
  properties,
}: {
  properties: ChecklistPropertyItem[];
}) {
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((p) => (
            <Link
              key={p.id}
              href={`/host/properties/${p.id}/checklist?returnTo=/host/checklists`}
